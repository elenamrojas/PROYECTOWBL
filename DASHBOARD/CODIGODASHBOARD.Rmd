...
# Cargar datos
library(here)
df <- readr::read_csv(here("DATOS", "sg_law_indx_clean.csv"))
...
```{r}
# Filtrar top 10 países 2023 (los de mayor puntuación)
top10_2023 <- df %>%
  filter(year == 2023) %>%
  arrange(desc(wbl_index)) %>%   # orden descendente
  slice(1:10) %>%
  pull(country)

df_top10 <- df %>% filter(country %in% top10_2023)
...
for(i in seq_along(top10_2023)) {
  pais <- top10_2023[i]
  fig <- fig %>% add_trace(
    data = df_top10[df_top10$country == pais, ],
    x = ~year,
    y = ~wbl_index,
    type = 'scatter',
    mode = 'lines',
...
fig %>% layout(
  title = "Evolución del índice legal: Top 10 países y promedio mundial",
